...
def handleButtons(channel):
    ...
    if channel == TURBO_PIN:
        client.write_gatt_char(CHARACTERISTIC_UUID, BT_MESSAGE_TURBO_ENABLED.encode("utf-8") if isRising else BT_MESSAGE_TURBO_DISABLED.encode("utf-8"))
    elif channel == POWER_ON_PIN:
        if not isRising:
            os.system("shutdown now -h")
    elif channel == PAUSE_BUTTON_PIN:
        client.write_gatt_char(CHARACTERISTIC_UUID, BT_MESSAGE_PAUSE_PRESSED.encode("utf-8") if isRising else BT_MESSAGE_PAUSE_RELEASED.encode("utf-8"))
    elif channel == TRIGGER_BUTTON_PIN:
        client.write_gatt_char(CHARACTERISTIC_UUID, BT_MESSAGE_TRIGGER_PRESSED.encode("utf-8") if isRising else BT_MESSAGE_TRIGGER_RELEASED.encode("utf-8"))
    elif channel == CURSOR_BUTTON_PIN:
        client.write_gatt_char(CHARACTERISTIC_UUID, BT_MESSAGE_CURSOR_PRESSED.encode("utf-8") if isRising else BT_MESSAGE_CURSOR_RELEASED.encode("utf-8"))
